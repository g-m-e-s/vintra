...
const processHandler = async (req, res) => {
  ...
  const { transcription, documentType, useVintraBackend } = req.body;
  ...
  try {
    ...
    if (useVintraBackend && documentType === 'vintra') {
      try {
        logger.info('Using Python backend for VINTRA analysis');
        const pythonResponse = await axios.post(`${PYTHON_API_URL}/api/vintra/analisar`, {
          transcricao: transcription,
          contexto_paciente: req.body.contexto_paciente || null
        });
        
        return res.status(200).json({
          success: true,
          content: pythonResponse.data,
          metadata: {
        ...
        logger.error('Error calling Python backend for VINTRA analysis', {
          error: pythonError.message,
          status: pythonError.response?.status
        });
        
        // Se a chamada ao Python falhar, continuamos com o processamento usando Claude diretamente
        logger.info('Falling back to Claude for VINTRA analysis');
      }
    }
